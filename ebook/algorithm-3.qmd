...
```{r}
#| code-fold: true
#| fig-cap: "In the background, level curves for $(x_1,x_2)\\mapsto m(0,x_1,x_2)$ and $m(1,x_1,x_2)$ respectively on the left and on the right. Then, on the left, individual $(s,x_1,x_2)=(s=0,-2,-1)$ (predicted 18.2% by model $m$, and 7.6% if $s$ is set to 1 leaving $x_1$ and $x_2$ unchanged), and on the right, visualization of two counterfactuals $(s=1,x_1^\\star,x_2^\\star)$ according to a causal graph where $x_1$ depends on $x_2$ (bottom right path, predicted 61.4%) and a causal graph where $x_2$ depends on $x_1$ (top left path, predicted 56.5%). The red square shows the counterfactual obtained with optimal transport (with a predicted value by model $m$ at 56.5%)."
