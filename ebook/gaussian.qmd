...
seq_functions <- transport_function(data = data, S_0 = S_0, 
...
transport_function2 <- function(data,
                                S_0,
                                n_grid = 15,
                                h = .2,
                                d = .5) {
  ...
  T1_cond_x2 <- function(x1, x2) {
    ...
    k1 <- which.min(abs(vx2_1_mid - T(x2)))
    i <- which.min(abs(vx1_0_mid - x1))
    p <- F1_0[i, k0]
    i <- which.min(abs(u - p))
    x1star <- Q1_1[i, k1]
...
n_grid <- 15
seq_functions_grids <- transport_function2(D_SXY, S_0 = S_0, n_grid = n_grid)
