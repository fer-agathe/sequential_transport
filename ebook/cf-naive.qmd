...
This chapter uses a naive approach to build counterfactuals: simply changing the sensitive attribute and leave the other characteristics unchanged (_ceteris paribus_ approach).
...
ggplot(
  aware_naive_black |> mutate(
    group = case_when(
      S_origin == "Black" & S == "Black" ~ "Black (Original)",
      S_origin == "Black" & S == "White" ~ "Black -> Black (counterfactual)",
      S_origin == "White" & S == "White" ~ "White (Original)"
    ),
    group = factor(
      group, 
      levels = c(
        "Black (Original)", "Black -> Black (counterfactual)", "White (Original)"
      )
    )
  ),
  ...
  scale_fill_manual(
    NULL, values = c(
      "Black (Original)" = colours_all[["source"]],
      "Black -> Black (counterfactual)" = colours_all[["naive"]],
      "White (Original)" = colours_all[["reference"]]
    )
  ) +
  facet_wrap(~S) +
