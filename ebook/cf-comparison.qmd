...
The predictions by the model using the factuals:
```{r redefine-factuals_aware} 
factuals_aware <- tibble(
  S = df_race_c$S,
  X1 = df_race_c$X1,
  X2 = df_race_c$X2,
...
get_prob <- function(type, model = "aware", ind, sensitive = "Black") {
  if (type == "factual") {
    ...
    x <- get(paste0("counterfactuals_", model, "_ot"))
  } else if (type == "seq") {
...
```{r define-subgroups}
ind_black <- which(df_race$S == "Black")
ind_white <- which(df_race$S == "White")
ind_pos <- which(df_race_c$Y == 1)
ind_neg <- which(df_race_c$Y == 0)
...
eq_op_pos <- NULL
eq_op_neg <- NULL
for (type in c("factual","ot", "fairadapt", "seq")) {
...
tb_res <- 
  ...
  mutate(type = ifelse(
    ...
    type = factor(type, levels = c("factual_white", "factual", "ot", "fairadapt", "seq"))
  ) |> 
  select(-sensitive) |> 
...
knitr::kable(
  tb_res[, -1], 
  col.names = c("Metric", "Factual", "Factual", "OT", "Fairadapt", "Seq. Transport")
) |> 
