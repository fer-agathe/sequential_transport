...
## Naive approcah: _ceteris paribus_
...
pred_unaware_naive <- predict(
  model_unaware,
  newdata = adult |> mutate(sex = "Male"),
  type = "response"
)
pred_aware_naive <- predict(
  model_aware,
  newdata = adult |> mutate(sex = "Male"),
  type = "response"
)

counterfactuals_unaware_naive <- 
  adult |> 
  mutate(
    pred = pred_unaware_naive,
    type = "counterfactual"
  )
counterfactuals_aware_naive <- 
  adult |> 
  mutate(
    pred = pred_aware_naive,
    type = "counterfactual"
  )
```
...
unaware_naive <- bind_rows(factuals_unaware, counterfactuals_unaware_naive)
unaware_naive_women <- unaware_naive |> filter(sex == "Female")
unaware_naive_men <- unaware_naive |> filter(sex == "Male")
```
...
#| fig-width: 8
# Ref: Men
ggplot(unaware_naive, aes(x = pred, fill = type)) +
  geom_histogram(
    mapping = aes(
...
aware_naive <- bind_rows(factuals_aware, counterfactuals_aware_naive)
aware_naive_women <- aware_naive |> filter(sex == "Female")
aware_naive_men <- aware_naive |> filter(sex == "Male")
```
...
#| fig-width: 8
# Ref: Men
ggplot(aware_naive, aes(x = pred, fill = type)) +
  geom_histogram(
    mapping = aes(
      y = after_stat(density)), alpha = 0.5,
    position = "identity", binwidth = 0.05
...
get_prob <- function(type, model = "aware", ind, sensitive = "Women") {
  if (type == "factual") {
    ...
    x <- get(paste0("counterfactuals_", model, "_naive"))
  } else {
    stop("Error type.")
  }
