...
This chapter introduces the scoring classifiers $m(\cdot)$ trained on the dataset described in @sec-data. We use logistic regression to predict the outcome $Y$ based on the covariates $\boldsymbol{X}$. Two types of models are considered: one that includes the sensitive attribute in the predictive model (**aware model**) and one that excludes it (**unaware model**).
...
Let us split the dataset into train/test sets (we use the `split_dataset()`{.R} function defined in our small package):
```{r define-train-test}
library(devtools)
...
The model is trained using the `log_reg_train()`{.R} function defined in our small package.
```{r}
log_reg_train
```
