...
```{r, dev.args = list(bg = 'transparent'), echo=FALSE, message=FALSE, warning=FALSE}
...
## Gaussian Probabilistis Graphical Models
...
In @fig-gauss-seq-rotation-1, we can visualize the set of all counterfactuals $\boldsymbol{x}^\star$ when $\theta\in[0,2\pi]$. The (global) optimal transport is also considered.
